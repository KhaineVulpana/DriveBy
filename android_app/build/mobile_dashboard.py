    ...
    * {
    ...
    def setup_routes(self) -> None:
        ...
        def get_keystrokes():
            ...
            return jsonify({"keystrokes": keystrokes[:50]})
        ...
        def disconnect_device_route(device_ip):
            """Disconnect device"""
            try:
    ...
    def is_device_online(self, device_ip: str) -> bool:
        """Check if device is currently online"""
        try:
            system = platform.system().lower()
            if "windows" in system:
                # Windows: -n count, -w timeout(ms)
                cmd = ["ping", "-n", "1", "-w", "1000", device_ip]
            else:
                # Linux/mac: -c count, -W timeout(s)
                cmd = ["ping", "-c", "1", "-W", "1", device_ip]
    ...
    def capture_device_screen(self, device_ip: str) -> bytes | None:
    ...
    def get_placeholder_screen(self) -> bytes:
        """Generate placeholder screen image"""
        # Simple base64 encoded 1x1 PNG placeholder image
        placeholder_b64 = """
        iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChAI9jU77zgAAAABJRU5ErkJggg==
        """
        return base64.b64decode(placeholder_b64.strip())

    def start_remote_control(self, device_ip: str) -> bool:
    ...
    def send_click_to_device(self, device_ip: str, x: float, y: float) -> bool:
        """Send click command to device"""
        try:
            data = {"x": x, "y": y}
    ...
    def send_key_to_device(self, device_ip: str, key: str) -> bool:
        """Send key command to device"""
        try:
            data = {"key": key}
