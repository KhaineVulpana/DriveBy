    ...
    def setup_routes(self):
        ...
        def store_credentials():
            ...
            try:
                ...
                filename = f"credentials_{service}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                ...
                print(
                    f"Stored {service} credentials for {data.get('email', 'unknown')} from {client_ip}"
                )

                return jsonify(
                    {"status": "success", "message": "Credentials stored for personal reference"}
                )
